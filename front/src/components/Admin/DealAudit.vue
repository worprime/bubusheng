...
<script>
  ...
  export default {
    ...
    mounted() {
      axios
        .get('http://localhost:8081/admin/getDealAudit', {
          params: {
            currentPage: 0,
            limit: 10
          }
    ...
    methods: {
      ...
      search() {
        if (this.key === '' ){
          ...
          axios
            .get('http://localhost:8081/admin/getDealAuditByKey', {
              params: {
                currentPage: this.currentPage,
                limit: this.pagesize,
                key: this.key,
      ...
      setData(data) {
        console.log(data);
        this.$router.push({path: "/Main/AuditDetail", query: {form: data, lastPathName: "待辨审核"}});
      }
    }
  }
</script>
